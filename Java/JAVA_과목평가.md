...
- 자식 Class의 기본 생성자는 부모 Class의 생성자를 별도로 호출하지 않으면 부모 Class의 기본 생성자를 호출. 하지만 부모 Class의 기본 생성자가 없으면 오류 발생
...
```text
sleep() : 일정 시간동안 실행을 중지
join() : 다른 Thread 개체 종료 시까지 대기
interrupt() : 대기중인 Thread 객체 깨우기
yield() : 다른 Thread들에게 수행 양보
wait() : 자원을 반납하고 Waiting Pool에서 대기
notify() 또는 notifyAll() : Waiting Pool에서 대기 중인 다른 Thread에게 이제 A자원이 다시 준비되었음을 알림

//

1. 어떤 Thread A가 공유 자원에 대한 Lock을 소유하더라도, 그 시점에 바로 중요 Job을 수행할 수 없으면, Lock을 계속 소유하지 않고 반납(Wait)하고 Waiting Pool 에서 기다림 -> 그럼, 공유자원을 원하는 다른 Thread들이 경쟁

2.  Thread A가 다시 Runnable 상태가 되면 다시 A의 Lock을 얻고 이후 동작 수행

3. Thread A가 공유자원으로 필요한 작업을 수행하였다면, Waiting Pool에서 해당 자원을 기다리며 대기 중인 다른 Thread에게 이제 공유자원이 다시 준비되었음을 알림(notify() 또는 notifyAll()) -> 알림을 받은 Thread들은 경쟁하면서 기회를 얻게 됨
```
